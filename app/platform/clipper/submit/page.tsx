...
export default function SubmitVideo() {
  ...
  const handleCampaignSelect = (campaignId: number) => {
    // Navigate to the submission form with the selected campaign
    window.location.href = `/platform/clipper/submit/${campaignId}`;
  };
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#dde2ee' }}>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-gray-900 mb-1">Submit New Video</h1>
          <p className="text-sm text-gray-600 mb-6">Choose a campaign below to get started</p>
        </div>

        {/* Campaign Selection - Exact same component as client platform */}
        <div className="space-y-6">
          {campaigns.map((campaign) => (
            <div
              key={campaign.id}
              onClick={() => handleCampaignSelect(campaign.id)}
              className="backdrop-blur-sm p-4 border border-white/20 transition-all duration-300 cursor-pointer group hover:scale-[1.02]" 
              style={{
                borderRadius: '1.5rem',
              ...
              role="button"
              tabIndex={0}
              aria-label={`Select ${campaign.name} campaign for video submission`}
              onKeyDown={(e) => {
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h2 className="text-2xl font-normal mb-2" style={{color: "rgba(0, 0, 0, 0.85)", letterSpacing: "-0.04em"}}>
                    {campaign.name}
                  </h2>
                </div>
          ...
          <div className="text-center py-12">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-gray-600 leading-relaxed">
                You don't have any active campaigns to submit videos to. Contact us to get started with your first campaign.
              </p>
            </div>
          </div>
        )}
